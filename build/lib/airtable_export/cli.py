...
@click.option(
    "--no-verify",
    is_flag=True,
    help="Disables SSL Verification when working behind enterprise firewall. Use with caution!",
)
...
def cli(
    ...
    if not tables or schema:
        # Fetch all tables
        schema_data = list_tables(base_id, key, user_agent=user_agent, verify=verify)
    ...
    for table in tables:
        ...
        try:
            ...
            for record in all_records(
                base_id, table, key, http_read_timeout, user_agent=user_agent, verify=verify
            ):
                r = {
...
def list_tables(base_id, api_key, verify, user_agent=None):
    url = f"https://api.airtable.com/v0/meta/bases/{base_id}/tables"
    headers = {"Authorization": "Bearer {}".format(api_key)}
    if user_agent is not None:
        headers["user-agent"] = user_agent
    return httpx.get(url, headers=headers, verify=verify).json()


def all_records(base_id, table, api_key, http_read_timeout, verify, sleep=0.2, user_agent=None):
    headers = {"Authorization": "Bearer {}".format(api_key)}
    if user_agent is not None:
        headers["user-agent"] = user_agent
    ...
    while first or offset:
        ...
        response = client.get(url, headers=headers, verify=verify)
